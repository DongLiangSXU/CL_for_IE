...
def train(net,rain_loader, loader_test,haze_loader, low_loader,optim, optim_ewc,pre_parms,criterion):
    ...
    grads_savepath = '/home/lyd16/PycharmProjects/wangbin_Project/AAAI2023works/' \
                     'CR_image_enhancement/import_npy_file/pre_rain2haze_cl.npy'
    ...
    replay_ratio = [0,0,0,1,1]

    print('-----------------------ewc prepare done!--------------------------------')

    # cl_for_lowlight
    totrain_loader = hazeloader
    for step in range(start_step+1,opt.steps+1):
        ...
        if isreplay or step % opt.eval_step ==0:
            ...
            with torch.no_grad():
                s1,_ = prenet(low_inputs)

            derain_re,_ = net(low_inputs)

            # with torch.no_grad():

            # s2,_ = prenet(derain_re)
        ...
        # print(f'\rtrain loss : {totalloss.item():.5f}|{(pixelloss.item()):.5f}|{(mycrloss.item()):.5f}|{(noforgetloss.item()):.5f}|'
        ...
        if step % opt.eval_step ==0 :
            with torch.no_grad():
                ...
                pthpath = './PRe_DD/pth_save/'+str(step)+'/'
                imgsavepath = './PRe_DD/imgsave/' + str(step) + '/'
...
if __name__ == "__main__":
    ...
    preTA = models_['P_PRE']
    prenet = preTA.to(opt.device)

    # depthnet = models_['depthnet']
    # depthnet = depthnet.to(opt.device)

    net.load_state_dict(torch.load('./trained_models/net_latest.pth'))
    prenet.load_state_dict(torch.load('./trained_models/net_latest.pth'))
