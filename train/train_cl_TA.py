...
def train(net,loader_train, loader_test,haze_loader, low_loader,optim, optim_ewc,pre_parms,criterion):
    ...
    grads_savepath = '/home/lyd16/PycharmProjects/wangbin_Project/AAAI2023works' \
                     '/CR_image_enhancement/import_npy_file/TA_rain2hazy_cl.npy'
    ...
    for step in range(start_step+1,opt.steps+1):
        ...
        print(f'\rtrain loss : {totalloss.item():.5f}|{(pixelloss.item()):.5f}|{(mycrloss.item()):.5f}|{(noforgetloss.item()):.5f}|{isreplay}|'
        ...
        if step % opt.eval_step ==0 :
            with torch.no_grad():
                ...
                pthpath = './HDM_TAC/pth_save/'+str(step)+'/'
                imgsavepath = './HDM_TAC/imgsave/' + str(step) + '/'
                ...
                torch.save(net.state_dict(), '%s/hdm_super_TA_%d.pth' % (pthpath, step))
...
if __name__ == "__main__":
    ...
    net.load_state_dict(torch.load('./trained_models/its_train_ffa_3_6.pk')['model'])
    prenet.load_state_dict(torch.load('./trained_models/its_train_ffa_3_6.pk')['model'])
