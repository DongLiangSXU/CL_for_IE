...
class Encoder_MDCBlock1(torch.nn.Module):
    def __init__(self, num_filter, num_ft, kernel_size=4, stride=2, padding=1, bias=True, activation='prelu', norm=None, mode='iter1'):
        ...
        for i in range(self.num_ft):
            ...
            # DeconvBlock(128, 64, 4, 2, 1, True, prelu, norm=None)
            # DeconvBlock(64, 32, 4, 2, 1, True, prelu, norm=None)
            # DeconvBlock(32, 16, 4, 2, 1, True, prelu, norm=None)

            self.up_convs.append(
                DeconvBlock(num_filter//(2**i), num_filter//(2**(i+1)), kernel_size, stride, padding, bias, activation, norm=None)
            )

            # ConvBlock(64, 128, 4, 2, 1, True, prelu, norm=None)
            # ConvBlock(32, 64, 4, 2, 1, True, prelu, norm=None)
            # ConvBlock(16, 32, 4, 2, 1, True, prelu, norm=None)
...
class Encoder_BMSFFBlock1(torch.nn.Module):
    def __init__(self, num_filter, num_ft, kernel_size=4, stride=2, padding=1, bias=True, wei=0.5,activation='prelu', norm=None, mode='iter1'):
        super(Encoder_BMSFFBlock1, self).__init__()
        self.mode = mode
        # 3
        self.num_ft = num_ft - 1
        self.wei = wei
        ...
        self.up_convs = nn.ModuleList()
        self.down_convs = nn.ModuleList()
        self.down_left_convs = nn.ModuleList()
        for i in range(self.num_ft):
    ...
    def forward(self, ft_l, ft_h_list):
        ...
        for i in range(len(ft_h_list)):
            ...
            for k in range(self.num_ft - i):
                left_ft = self.down_left_convs[self.num_ft - i - k - 1](left_ft)

            left_res = oft-left_ft
            ft = self.wei*left_res+(1-self.wei)*ft
...
class Decoder_BMSFFBlock1(torch.nn.Module):
    def __init__(self, num_filter, num_ft, kernel_size=4, stride=2, padding=1, bias=True, wei=0.5,activation='prelu', norm=None, mode='iter1'):
        super(Decoder_BMSFFBlock1, self).__init__()
        self.mode = mode
        ...
        for i in range(self.num_ft):
            ...
            # ConvBlock(32, 64, kernel_size, stride, padding, bias, activation, norm=None)
            # ConvBlock(64, 128, kernel_size, stride, padding, bias, activation, norm=None)

            # ConvBlock(128, 256, kernel_size, stride, padding, bias, activation, norm=None)
            ...
            # DeconvBlock(64, 32, kernel_size, stride, padding, bias, activation, norm=None)
            # DeconvBlock(128, 64, kernel_size, stride, padding, bias, activation, norm=None)
            # DeconvBlock(256, 128, kernel_size, stride, padding, bias, activation, norm=None)
    ...
    def forward(self, ft_h, ft_l_list):
        ...
        for i in range(len(ft_l_list)):
            ...
            for k in range(self.num_ft - i):
                left_ft = self.up_left_convs[self.num_ft - i - k - 1](left_ft)
